...
const existingAttachments = ref([...props.question.attachments]);
...
const submit = () => {
    ...
    form.transform((data) => ({ ...data, _method: 'put' })).post(route('questions.update', props.question.id), {
        forceFormData: true,
        onSuccess: () => {
            selectedFiles.value = [];
        },
    ...
    <AuthenticatedLayout>
        <section class="flex flex-col gap-6">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-3xl font-semibold">Edit Question</h1>
                    <p class="mt-2 text-sm text-slate-400">Update details and keep the thread tidy.</p>
                </div>
                <Link
            ...
            <form class="flex flex-col gap-6" @submit.prevent="submit">
                <div class="rounded-2xl border border-slate-800 bg-slate-950/50 p-6">
                    ...
                    <TextInput
                        ...
                        type="text"
                        class="mt-2 w-full"
                        placeholder="Summarize the question in one line"
                    />
