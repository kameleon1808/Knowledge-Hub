...
const aiAnswerPending = ref(false);
...
const generateAiAnswer = async () => {
    if (!questionData.value.can?.generate_ai_answer || aiAnswerPending.value) return;
    aiAnswerPending.value = true;
    try {
        const response = await window.axios.post(
            route('questions.ai-answer', questionData.value.id)
        );
        const answer = response.data.answer;
        if (answer && !answersData.value.some((a) => a.id === answer.id)) {
            answersData.value.push({ ...answer, isNew: true });
            newAnswerHighlightId.value = answer.id;
            setTimeout(() => {
        ...
        const msg =
            ...
            'Unable to generate AI answer. Please try again.';
        alert(msg);
    } finally {
        aiAnswerPending.value = false;
    }
};
        ...
        <section class="flex flex-col gap-8">
            <div class="flex items-start justify-between gap-6">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <button
                        v-if="questionData.can.generate_ai_answer"
                        type="button"
                        class="rounded-full border border-violet-500/60 px-4 py-2 text-sm text-violet-200 transition hover:border-violet-400 hover:text-violet-100 disabled:opacity-50"
                        :disabled="aiAnswerPending"
                        @click="generateAiAnswer"
                    >
                        {{ aiAnswerPending ? 'Generating…' : 'Generate AI Answer (Draft)' }}
                    </button>
                    <Link
                            ...
                            <div class="flex-1">
                                <div class="flex items-start justify-between gap-4">
                                    <div class="flex flex-wrap items-center gap-3 text-xs uppercase tracking-[0.18em] text-slate-500">
                                        ...
                                        <span
                                            v-if="answer.ai_generated"
                                            class="rounded-full border border-violet-400/60 bg-violet-400/10 px-3 py-1 text-[10px] text-violet-200"
                                            title="Generated by AI"
                                        >
                                            AI
                                        </span>
                                        <span
                                            v-if="answer.ai_generated && answer.id === newAnswerHighlightId"
